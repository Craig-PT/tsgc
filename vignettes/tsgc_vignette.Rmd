...
`tsgc` is a package for `R` that is intended for use in monitoring and forecasting the progress of an epidemic, including the detection *new waves* and *turning points*.  It develops and implements time series growth curve methods first reported in @Harvey2020 (hereinafter referred to as HK). HK develop a class of time series models for predicting future values of a variable which, when cumulated, is subject to an unknown saturation level. In a single wave of an epidemic, as more and more people get infected, the pool of susceptible individuals dwindles. This results in the decline of new infections, and the cumulative number of infections approaches its saturation level. The model can take account of deviations relative to this canonical trajectory due to  changes in  social behavior and policy. Models in this family are relevant for many other disciplines, such as marketing (when estimating the demand for new products and  innovations). While attention here is focused on the spread of epidemics and the applications used for illustration relate to coronavirus, this package is designed with a view to wider applicability.
...
In contrast, the empirical approach implemented in `tsgc` falls into the class of models described as `phenomenological'. Although it is motivated by the archetypal pattern in the dynamics of disease spread, our approach does not rely on structural assumptions derived from epidemiological theory. There are advantages to not requiring assumptions about values of parameters relating to, inter alia, disease infectiousness, disease severity, or contact patterns, which are difficult to pin down with sufficient precision, especially in real-time during an epidemic. Our approach makes minimal assumptions and merely requires historical observations of the epidemic variable to be forecast, to which we apply time-series methods to provide predictions over short future time horizons. The model can be estimated quickly and straightforwardly, and subjected to standard diagnostic tests. A statistical model of this type is a useful complement to models that attempt to describe the epidemic in terms of underlying processes mechanistically.

The next section sets out the state space formulation of the dynamic Gompertz growth curve and the way forecasts are obtained from predictive recursions. Section 3 explains how multiple waves can be accommodated by reinitializing the series at the starts of new waves. The start of a new wave is not obvious in real time but a rule for triggering reinitialization that works well in practice is presented. Section 4 describes the functionality of `tsgc`. Section 5 sets out a full working example of the use of the package to forecast COVID infection in Gauteng province in South Africa. Section 6 concludes.

# Theory

## Gompertz curve

Our model is grounded on the sigmoidal growth curve pattern that characterizes epidemics. We start by assuming that the cumulative number of cases follows a Gompertz curve, which is a parsimonious model for the canonical sigmoid shape of cumulative case numbers in a one-wave epidemic. Over the course of a wave, the number of new infected cases increases up to a peak before declining to zero as the pool of susceptible individuals declines. Specifically, if the cumulative number of cases at time $t$, $\mu(t)$, follows a Gompertz curve, we can write $$\mu(t) = \bar{\mu}\exp\{\gamma_0 e^{\gamma t}\},$$ where $\bar{\mu}$ is the unknown saturation level for the cumulative number of cases, $\gamma_0 < 0$ is a parameter related to $\mu(0)$ and $\gamma < 0$ is the growth rate parameter. Defining $\dot{\mu}(t) \equiv d\mu(t)/dt$ and $g(t) \equiv {\dot\mu(t)}/{\mu(t)}$, it is straightforward to show that

\begin{equation}
\ln g(t) = \delta + \gamma t,
\end{equation} where $\delta = \ln \gamma_0 \gamma$.
...
The deterministic trend implied by the static Gompertz curve is too inflexible for practical time-series modeling of an epidemic. Replacing it with a stochastic trend allows the model to adapt to changes in dynamics during  the course of the epidemic. We call this stochastic-trend counterpart of the static Gompertz curve the dynamic Gompertz model. It is a local linear trend model specified as

\begin{equation}
\ln g_{t}=\delta_{t}+\varepsilon_{t},\text{     }\varepsilon_{t}\sim
NID(0,\sigma_{\varepsilon }^{2}),\text{         }t=2,...,T,  
\end{equation} where $\ln g_{t}=\ln y_{t}-\ln Y_{t-1}$ and
\begin{eqnarray}
\delta_{t} &=&\delta_{t-1}+\gamma_{t-1},  \\
\gamma_{t} &=&\gamma_{t-1}+\zeta_{t},\text{       }\zeta_{t}\sim
NID(0,\sigma_{\zeta }^{2}),
...
$q_{\zeta }=\sigma_{\zeta }^{2}/\sigma_{\varepsilon }^{2}$, the faster the estimate of the slope parameter, $\gamma$, which can be interpreted as the growth rate of the growth rate of cumulative cases, changes in response to new observations. Conversely, a lower signal-to-noise ratio induces more smoothness to the estimate. When $\sigma_{\zeta }^{2}=0$, the trend is deterministic as in the static Gompertz curve.
...
Forecasts of future observations can be obtained from the predictive recursions 

\begin{eqnarray*}
\widehat{g}_{T+\ell \mid T} &=&\exp (\hat\delta_{T\mid T}+\hat\gamma
_{T\mid T}\ell ),\text{      }\ell =1,2,..  \label{formu} \\
\widehat{\mu }_{T+\ell \mid T} &=&\widehat{\mu }_{T+\ell -1\mid T}(1+
\widehat{g}_{T+\ell \mid T}) %\text{        }
\end{eqnarray*} 
...
_{T+\ell -1\mid T}  \\
\label{eq:yhat}
&= Y_T \exp\hat\delta_{T+\ell|T} \prod_{j=1}^{\ell-1}(1+\exp \hat\delta_{T+j|T}) 
\end{align} 

and  $\widehat{Y}_{T+\ell \mid T}=\widehat{\mu }_{T+\ell \mid T};$ the initial value is $\widehat{\mu }_{T\mid T}=Y_{T}.$

We construct forecast intervals for $y_t$ based on the prediction intervals for $\delta_t$. The conditional distribution of future values of $\hat\delta_{t}$ is  Gaussian. We replace $\hat\delta_{T+j|T}$ in the predictive recursions with the upper bound of a prediction interval for $\hat\delta_{T+j|T}$ to compute the upper bound of our forecast interval for $y_{T+\ell}$ and likewise for the lower bound.^[These are not proper prediction or confidence intervals for $y_{T+\ell}$. The one-step-ahead predictive distribution of $\hat{y}_{T+\ell|T}$ (for $\ell=1$) is lognormal. This is not the case more than one step ahead due to the presence of the cumulative total in the dynamic Gompertz model.]  In effect, the forecast intervals are based on inference on the log cumulative growth rate, $\delta_{t}$.

The filtered growth rate $\hat{g}_{y,t\mid t}$ of new cases $y_{t}$, can be extracted from the continuous-time incidence curve: $\mu^{\prime}(t) = g(t) \mu(t)$, where $\mu(t)$ is the growth curve and $g(t)$ is its growth rate. Taking logarithms and differentiating

\begin{equation}
\hat{g}_{y,t\mid t}=\hat{g}_{t\mid t}+\hat\gamma_{t\mid t},%\text{   }  %
\label{eq:gyt}
\end{equation} 

where $\hat{g}_{t\mid t}=\exp \hat\delta_{t\mid t}.$ The sampling variability of $\hat{g}_{t\mid t}$ is dominated by that of $\hat\gamma_{t\mid t}$ [@Harvey2021a]. Therefore when constructing confidence intervals for $\hat{g}_{t\mid t}$ we treat $\hat{g}_{y,t}$ as if it has a normal distribution centered on $\hat{g}_{y,t\mid t}$ with variance $\text{Var}(\hat\gamma_{t\mid t})$.
...
\begin{equation}
\exp \hat\delta_{T\mid T}+\hat\gamma_{T\mid T}=\hat{g}_{T\mid T}+\hat\gamma_{T\mid T}.%\text{   }  %
\label{eq:grdaily}
\end{equation} 


When $\hat\gamma_{T\mid T}$ is negative, there is a flattening of the curve and a signaling of an upcoming peak in the trend of $y_{t}.$ As shown in [HK, p10], the peak in the trend is predicted to be $\ell_{T}$ days ahead where^[Note the change in sign of $\gamma$ as compared with HK.] 
$$
\ell_{T}=\frac{\ln (-\hat\gamma_{T\mid T})-\hat\delta_{T\mid T}}{\hat\gamma_{T\mid T}}=\frac{\ln (-\hat\gamma_{T\mid T}/\hat{g}_{T\mid T})}{\hat\gamma_{T\mid T}},\text{        }
...
The path of the epidemic is best tracked by nowcasts and forecasts of $g_{y,t}$, the growth rate of $y_t$, which are constructed by HK from the filtered estimates in the state space model. @WallingaLipsitch2007 describe how the estimates of $g_{y,t}$ can be translated into estimates of the instantaneous reproduction number $R_{t}.$ @Harvey2021a establish the most suitable translation as

\begin{equation}
\widetilde{R}_{t,\tau}=1+\tau g_{y,t\mid t}\quad\text{or}\quad\widetilde{R}_{\tau,t}^{e}=\exp(\tau g_{y,t\mid t}),
\end{equation}
where $\tau$ is the generation interval -- the typical number of days between an infected person becoming infected and them transmitting the disease to someone else. We construct credible intervals for $\widetilde{R}_{t,\tau}$ and $\widetilde{R}_{\tau,t}^{e}$ by substituting the upper and lower bounds of the confidence intervals for $g_{y,t}$ into the equation for $\widetilde{R}_{\tau,t}^{e}$ to get the upper and lower bounds of the credible intervals. See @Harvey2021b for an application. 

The estimates of $R_{t}$ are a by-product of the estimates of $g_{y,t}$, which are sufficient for tracking and forecasting the epidemic. The nowcasts of $y_{t}$ peak when $\hat{g}_{y,t\mid t}=0$, corresponding to $\widetilde{R}_{t,\tau}=\widetilde{R}_{\tau,t}^{e}=1.$ 
Predictions of $g_{y,t}$ are given by

\begin{equation}
\hat{g}_{y,T+\ell\mid T}=\exp\hat\delta_{T+\ell\mid T}+\hat\gamma_{T+\ell\mid T}=\exp(\hat\delta_{T\mid T}+\hat\gamma_{T\mid T}\ell)+\hat\gamma_{T\mid T},\quad \ell=1,2,.
\end{equation}

We can then obtain predictions of $R_{t}$. If $\hat\gamma_{T\mid T}$ is zero, the estimated growth of $y_{t}$ is exponential and it is helpful to characterize it by the doubling time, $\ln2/\hat{g}_{y,T\mid T} =0.693\exp(-\hat\delta_{T\mid T}).$  

When $\exp \hat\delta _{T\mid T}+\hat\gamma _{T\mid T}>0$, the nowcast $\hat{g}_{y,T\mid T}$ is positive and the estimate of $R_{t}$ is greater than one. So long as $\hat\gamma _{T\mid T}$ is negative, then as $T\rightarrow \infty, \text{ } \widetilde{R}_{\tau ,T+\ell \mid T}^{e}\rightarrow \exp (\tau\hat\gamma _{T\mid T})<1$, and a saturation level for $Y$ appears on the horizon. 

We now turn to case where $\gamma_{t}$ potentially turns positive in a typically short-lived phase, as a new wave emerges. 

# Reinitialization

The coronavirus pandemic was characterized by multiple waves punctuated by plateaus. At the beginning of a new wave the growth rate of daily cases, $g_{y,t}$, will turn positive.  In some cases, the initial surge may be explosive to the point where the growth is *super exponential*. In this case,  $\gamma _{t}$, the growth rate of $g_t$ (which is the growth rate of cumulative cases) can also turn positive, with no peak in prospect for $y_{t}.$  Such a  phase can be expected to be transient, with $\gamma _{t}$ dropping back to zero (exponential growth in infection, accompanied by an upcoming peak in $y_{t}$), and then falling below zero (sub-exponential growth in infection).  
...
\begin{equation}\label{eq:gg0}
\ln g_t^r = \ln g_t +  \ln \frac{Y_{t-1}}{Y_{t-1}^r}  
= \ln g_t +  \ln \frac{Y_{t-1}}{Y_{t-1} - Y_{r_0}} \;\;\;\; t=r+1, \ldots, T
\end{equation}
...
These implement the models described in the Theory section, with and without reinitialization, respectively.  They both inherit from a common base class `SSModelBase`, which acts as a wrapper around `KFAS` to set up the state space model and define consistent update and estimation methods for it.
...
- `file.path` sets the root directory where the sub-folders for saved results will be created. These sub-folders will be created within the working directory from where this script is stored and run. This can be retrieved using `here::here()`
  
-  `date.format` specifies the format for dates. In this example, our dates are in the form `\%Y-\%m-\%d`, e.g. 1 December 2021 is `2021-12-01`. If the dates were in the form `\%d\%m\%Y`, 1 December 2021 would be `01122021`.
...
The reproduction numbers \(R_t\) and their 68\% credible intervals can be calculated. The plot in Figure 5 reveals that \(R_t\) remains above one through the period, indicating that the (third) wave in Gauteng had launched by this time. 
...
A CSV file (named `y-forecast`) can be written to the directory specified using the code below. The forecast options specified earlier are retained. 
...
It is not obvious, a priori, when precisely to reinitialize the model. Based on experiments a sensible option is to reinitialize when the estimated slope parameter, $\gamma_t$, breaches a threshold of two standard errors above zero, and at that point backdate reinitialization to when the $\gamma_t$ first turned positive. In applying the above heuristic there is a choice between the filtered slope estimate and the smoothed slope estimate. Experiments suggests the greater noisiness of the filtered estimate of $\gamma_t$ often triggers reinitialization too early. The smoothed estimate provides a more reliable trigger. 

Figure 6 shows that for the $3^{rd}$ wave in Gauteng, the smoothed slope estimate exceeded twice its standard error on 3 May 2021, having risen above zero on 29 April 2021.
...
While the package is demonstrated using COVID-19 data from South Africa, it can be used to model and forecast any time series variable where a growth curve-like trajectory that potentially nests multiple waves in succession, is expected, and where forecasting future observations is of interest - for example, sales of a new product, innovation adoption, or website traffic, among others.


# Acknowledgements {.unnumbered}

We thank The Cambridge Centre for Health Leadership \& Enterprise, University of Cambridge Judge Business School, and Public Health England/UK Health Security Agency for generous support. AH is grateful to the University of Cambridge Keynes Fund for support on the project Persistence and Forecasting in Climate and Environmental Econometrics.
...
When we add a seasonal term to the model, the observation equation in the dynamic Gompertz curve becomes
$$
\ln g_{t}=\delta_{t}+\nu_t+\varepsilon_{t},\text{     }\varepsilon_{t}\sim
NID(0,\sigma_{\varepsilon }^{2}),\text{         }t=s,...,T,
$$
where $\nu_t$ is the seasonal component, $\delta_t$ remains defined as before, and $\varepsilon_t$ remains the iid Normal disturbance. 
...
where
\begin{align}
\nu_{j,t} &= \nu_{j,t-1} \cos \lambda_j + \nu^*_{j,t-1} \sin \lambda_j + \omega_{j,t} \\
\nu^*_{j,t} &= -\nu_{j,t-1} \sin \lambda_j + \nu^*_{j,t-1} \cos \lambda_j  + \omega^*_{j,t}, \;\;\;\;\; j = 1, \ldots, s^*,
\end{align}
